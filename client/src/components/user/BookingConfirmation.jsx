...
const generateOrderId = (mongooseId) => {
  ...
  const hash = crypto.createHash("md5").update(mongooseId).digest("hex");
  const shortHash = hash.slice(0, 8).toUpperCase();

  return `ORD-${shortHash}`;
};
...
const BookingConfirmation = () => {
  ...
  useEffect(() => {
    if (location.state?.bookingDetails) {
      ...
      setConfirmationNumber(`${type.toUpperCase()}-${Math.floor(100000 + Math.random() * 900000)}`);
      setLoading(false);
    } else {
  ...
  if (error || !bookingDetails) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center bg-gray-100">
        ...
        <button onClick={() => navigate("/")} className="mt-4 rounded-md bg-blue-600 px-4 py-2 font-medium text-white transition duration-150 ease-in-out hover:bg-blue-700">
          Go to Homepage
        </button>
      </div>
    );
        ...
        <div className="overflow-hidden rounded-lg bg-white shadow-md">
          ...
          <div className="p-6">
            ...
            {type === "hotel" ? (
              ...
              <div>
                <div className="mb-4 border-b pb-4">
                  ...
                  <p className="text-gray-700">
                    <span className="font-medium">Route:</span> {bookingDetails.departureCity} to {bookingDetails.arrivalCity}
                  </p>
                  <p className="text-gray-700">
                  ...
                  <p className="text-gray-700">
                    <span className="font-medium">Class:</span> {bookingDetails.class.charAt(0).toUpperCase() + bookingDetails.class.slice(1)}
                  </p>
                </div>
                ...
                <div className="mb-4 border-b pb-4">
                  ...
                  {bookingDetails.passengers?.map((passenger, index) => (
                    <div key={index} className={index > 0 ? "mt-4" : ""}>
                      <p className="text-gray-700">
                        <span className="font-medium">Passenger {index + 1}:</span> {passenger.firstName} {passenger.lastName}
                      </p>
                      {index === 0 && (
                        <>
                          <p className="text-gray-700">
            ...
            <div>
              ...
              <p className="text-gray-700">
                <span className="font-medium">Payment Status:</span> <span className="font-medium text-green-600">Paid</span>
              </p>
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-6">
            <div className="flex flex-col items-center justify-between sm:flex-row">
              ...
              <div className="flex space-x-4">
                <button onClick={() => navigate("/profile")} className="rounded-md bg-blue-600 px-4 py-2 font-medium text-white transition duration-150 ease-in-out hover:bg-blue-700">
                  View All Bookings
                </button>
              </div>
            </div>
