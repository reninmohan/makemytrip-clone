...
export function UserBookingSection() {
    ...
    const fetchBookings = async () => {
      try {
        ...
        const [hotelRes, flightRes] = await Promise.all([
          api.get("/api/users/bookings/hotels"),
          api.get("/api/users/bookings/flights"),
        ]);
  ...
  const handleFlightlDelete = async (bookingId) => {
    try {
      await api.delete(`/api/users/bookings/flight/${bookingId}`);
      toast.success("Hotel booking deleted");
              ...
              {flightBookings.length === 0 ? (
                ...
                flightBookings.map((booking) => (
                  <FlightBookedCard
                    key={booking._id}
                    booking={booking}
                    onDelete={() => handleFlightlDelete(booking._id)}
                  />
                ))
              )}
            </TabsContent>
